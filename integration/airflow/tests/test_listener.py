...
@patch("openlineage.airflow.utils.is_airflow_version_enough")
@patch("openlineage.airflow.listener.execute_in_thread")
@patch.dict(os.environ, {"OPENLINEAGE_AIRFLOW_ENABLE_DIRECT_EXECUTION": "true"})
def test_listener_chooses_direct_execution_when_env_variable(
    is_airflow_version_enough, execute_in_thread
):
...
@patch("openlineage.airflow.utils.is_airflow_version_enough")
@patch("openlineage.airflow.listener.execute_in_thread")
@patch.dict(os.environ, {"OPENLINEAGE_AIRFLOW_ENABLE_DIRECT_EXECUTION": "false"})
def test_listener_chooses_thread_execution(is_airflow_version_enough, execute_in_thread):
    is_airflow_version_enough.return_value = False
    is_called = False
